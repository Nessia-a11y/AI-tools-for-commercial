...
async def auth_login(req: LoginRequest, request: Request):
    ...
    code = auth.generate_otp(req.email)
    sent = auth.send_otp_email(req.email, code)
    auth.log_login(req.email, "otp_sent", ip, ua, success=sent)
...
async def auth_verify_otp(req: OTPVerifyRequest, request: Request):
    ...
    if not auth.verify_otp(req.email, req.code):
        auth.log_login(req.email, "otp_fail", ip, ua, success=False)
        raise HTTPException(401, "Invalid or expired verification code")

    token = auth.create_session(req.email, ip, ua)
    auth.log_login(req.email, "login_success", ip, ua, success=True)
